...
import { requestDedup, responseCache } from '@/utils/requestCache'
...
export async function apiFetch<T>(
  ...
  // 生成缓存键
  const cacheKey = `api:${method}:${url}:${JSON.stringify(init.body || '')}`

  // GET 请求：检查缓存
  const isGetRequest = method === 'GET'
  const enableCache = options?.cache !== (isGetRequest ? false : undefined)
  const cacheMaxAge = options?.cacheMaxAge ?? 60000

  if (isGetRequest && enableCache !== false) {
    const cached = responseCache.get(cacheKey, cacheMaxAge)
    if (cached !== null) {
      return cached
    }
  }
  ...
  // GET 请求使用去重，非 GET 请求直接执行
  const result = isGetRequest
    ? await requestDedup.request(cacheKey, fetcher)
    : await fetcher()

  // GET 请求：写入缓存
  if (isGetRequest && enableCache !== false) {
    responseCache.set(cacheKey, result, cacheMaxAge)
  }
